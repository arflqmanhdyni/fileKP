	...
	curl_setopt_array($curl_pos, array(
		...
		CURLOPT_POSTFIELDS => "origin=375&destination=".$id_kabupaten."&weight=".$berat."&courier=pos",
	...
	curl_setopt_array($curl_jne, array(
		...
		CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		CURLOPT_CUSTOMREQUEST => "POST",
		CURLOPT_POSTFIELDS => "origin=375&destination=".$id_kabupaten."&weight=".$berat."&courier=jne",
		CURLOPT_HTTPHEADER => array(
			"content-type: application/x-www-form-urlencoded",
			"key: 8f22875183c8c65879ef1ed0615d3371"
		),
	...
	curl_setopt_array($curl_tiki, array(
		...
		CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		CURLOPT_CUSTOMREQUEST => "POST",
		CURLOPT_POSTFIELDS => "origin=375&destination=".$id_kabupaten."&weight=".$berat."&courier=tiki",
		CURLOPT_HTTPHEADER => array(
			"content-type: application/x-www-form-urlencoded",
			"key: 8f22875183c8c65879ef1ed0615d3371"
		),
				...
				for($a = 0;$a < count($data_jne['rajaongkir']['results'][0]['costs']);$a++){
					...
					<tr>
						...
						<td class="text-center">
							<input type="radio" name="pil_kurir" class="pilih-kurir" kurir="JNE" service="<?php echo $data_jne['rajaongkir']['results'][0]['costs'][$a]['service']; ?>" harga="<?php echo $data_jne['rajaongkir']['results'][0]['costs'][$a]['cost'][0]['value']; ?>" required="required">
						</td>
					</tr>
					<?php 
				}
			...
			if(count($data_pos['rajaongkir']['results'][0]['costs']) > 0){
				for($a = 0;$a < count($data_pos['rajaongkir']['results'][0]['costs']);$a++){
					...
					<tr>
						...
						<td class="text-center">
							<input type="radio" name="pil_kurir" class="pilih-kurir" kurir="Pos Indonesia" service="<?php echo $data_pos['rajaongkir']['results'][0]['costs'][$a]['service']; ?>" harga="<?php echo $data_pos['rajaongkir']['results'][0]['costs'][$a]['cost'][0]['value']; ?>" required="required">
						</td>
					</tr>
					<?php 
				}
			...
			if(count($data_tiki['rajaongkir']['results'][0]['costs']) > 0){
				for($a = 0;$a < count($data_tiki['rajaongkir']['results'][0]['costs']);$a++){
					...
					<tr>
						...
						<td class="text-center">
							<input type="radio" name="pil_kurir" class="pilih-kurir" kurir="TIKI" service="<?php echo $data_tiki['rajaongkir']['results'][0]['costs'][$a]['service']; ?>" harga="<?php echo $data_tiki['rajaongkir']['results'][0]['costs'][$a]['cost'][0]['value']; ?>" required="required">
						</td>
					</tr>
					<?php 
				}
